...
> **Reflects:** v6.2 — **Intentions Backlog** (+ Todoist unschedule-on-discard fix). Intentions discarded during planning move to a persistent `life.backlog` rather than being lost. Day rollover auto-saves the stale plan to `history` (authoritative — replaces any same-date manual save) and harvests unfinished intentions (those with uncompleted linked tasks) into the backlog with `reason: 'rollover'`. Intention rows in Step 1 + Step 3 get two icon buttons — `📥` Move to backlog (non-destructive) and `🗑` Delete (confirm-modal-gated). Both paths now correctly unschedule linked Todoist tasks (fixing a latent v6.1 bug where `REMOVE_INTENTION` left stale `due_*` fields in Todoist). A new `HistorySidebar` (renamed from `SavedSessions`) hosts the existing Saved Sessions UI alongside a new Backlog tab; Wizard + Dashboard headers each get a second `📥 Backlog (N)` button. Schema bumps to `6.2`. Iteration 7 (modes/rituals/recovery) and iteration 8 (reviews/drift) remain sketched in [history/plan_v5.md](./history/plan_v5.md).
>
> **v6.1** decoupled stabilizer habits from intentions: creating a stabilizer syncs a recurring Todoist task into a project the user picks (workspace default lives at `AppSettings.habitsTodoistProjectId`; per-habit override at `Habit.todoistProjectId`; when neither is set, a project named "Habits" is lazily auto-created), and during planning the habit's task is surfaced **directly as an orphan LinkedTask** (no `intentionId`) and auto-assigned to whichever session contains its Todoist `due.datetime`. Editing a habit's project moves the existing recurring task via the Sync API (`item_move`). Per-habit `targetTime` / `targetDurationMinutes` / `windowBehavior` replaced the deprecated `autoLinkTodoistId` / `maxBlockMinutes`. `LinkedTask.intentionId` became optional + `sourceHabitId` was added; `INJECT_HABIT_INTENTIONS` / `SKIP_HABIT_INTENTION` became `INJECT_HABIT_TASKS` / `SKIP_HABIT_TASK`.
>
...
| Term | Meaning |
|---|---|
| **Intention** | A high-level goal for *today* (e.g. "Finish assignment 3"). Not a todo-list epic — a today-scoped focus area. Owned by Orchestrate. v6.1: intentions only come from the user; stabilizer habits no longer auto-inject as intentions. v6.2: discarded intentions can move to the **Backlog** instead of being deleted. |
...
| **Backlog** | v6.2: a persistent pool of parked intentions, stored at `life.backlog`. Populated two ways: (a) user clicks `📥` on an intention in Step 1 / Step 3 (`reason: 'manual'`), or (b) day rollover harvests yesterday's intentions whose linked tasks weren't all completed (`reason: 'rollover'`). Surfaces in the second tab of the `HistorySidebar`. Per-entry: "Bring to today" (re-creates the intention with fresh unclassified `LinkedTask` rows) or "Discard" (confirm-modal, unschedules Todoist tasks). Manual move-to-backlog also unschedules Todoist; rollover leaves Todoist alone so yesterday's tasks remain visibly overdue there. |
...
The plan auto-resets daily. **v6.2:** the init path is now a coordinated `loadInitialState()` helper that peeks at the persisted plan via `peekRawPlan()` — when the date is stale, it auto-saves the plan to `history` with `label: "Auto: {date}"` (authoritative — replaces any same-date manual save) and harvests unfinished intentions into `life.backlog`. User preferences (Todoist token, session slots, calendar IDs, **`habitsTodoistProjectId`** in v6.1) survive in `AppSettings`. Completed days can also be manually saved to `history` and restored later.
...
**Persistence & history**
- Auto-reset of plan when date changes — **v6.2:** also auto-saves the stale plan to `history` (label `Auto: <date>`, authoritative — replaces any same-date manual save) and harvests unfinished intentions into the backlog
...
- Day rollover harvests unfinished intentions (those with uncompleted linked tasks) automatically.
- Surfaces in the second tab of the renamed `HistorySidebar` (was `SavedSessions`). Wizard + Dashboard headers get a second `📥 Backlog (N)` button that opens the same sidebar focused on the Backlog tab.
- **"Bring to today"** re-creates the intention with fresh `LinkedTask` rows (`type: 'unclassified'`, no estimate, no assignment) — user re-flows them through Step 2 + Step 3.
