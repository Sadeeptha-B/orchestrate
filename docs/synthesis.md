...
>
> **Last updated:** 2026-05-17 · **Schema:** v6.3 · **Iteration history:** [history/plan_v6.x.md](./history/plan_v6.x.md)
...
| **LinkedTask** | A Todoist task surfaced inside Orchestrate's plan, bound to an intention via `intentionId`. v6.3: stabilizer habits no longer live here — they're `TodaysHabitInstance` entries. Each LinkedTask now carries a `status` + optional `engagement` record. |
| **Backlog** | Persistent pool of parked intentions at `life.backlog`. Populated via manual `📥` or day-rollover harvest. Surfaces in the `HistorySidebar`'s Backlog tab. v6.3: backlog entries also preserve engagement records from previously-engaged tasks (`unfinishedTaskRecords`). |
| **Engagement** | v6.3: explicit Start/Stop tracking on a LinkedTask or `TodaysHabitInstance`. Captured via ▶ / ■ buttons on the dashboard; accumulates minutes across cycles. Engaged tasks moved to the backlog carry their record forward as a read-only memo on the BacklogEntry. |
| **Main task** | A primary work thread. Exclusive to one session. |
| **Background task** | A small/recurring task. Can be assigned to multiple sessions. Cap resolved from `AppSettings.taskCapDefaults.manualBackground` (default 30 min). |
...
| **Stabilizer** | `kind: 'stabilizer'` habit. Synced to Todoist as a recurring task. v6.3: surfaces as a `TodaysHabitInstance` positioned on the timeline by `targetTime`, independent of session assignment. Has its own Start/Stop/Complete/Skip/Reschedule lifecycle. |
| **TodaysHabitInstance** | v6.3: a stabilizer's manifestation for today. Lives on `DayPlan.todaysHabits[]`. Owns `status` (planned/engaged/completed/unfinished/skipped), `engagement`, and `rescheduledFromId` for the clone-on-reschedule chain. Never enters session capacity. |
...
1. **Step 1 — Intentions.** Two phases: (a) write down intentions for the day, (b) sequentially walk through each intention and *map* it to specific Todoist tasks via the embedded TodoistPanel ("Link"/"Unlink" buttons). Mapped intentions become collapsible panels showing their linked tasks; users can remap individually or restart mapping wholesale. **v6.3:** stabilizer habits no longer appear in the intention list — instead, the step mounts a refresh effect (`REFRESH_TODAYS_HABITS`) that adds today's stabilizer habits to `plan.todaysHabits` as `TodaysHabitInstance` rows. An informational chip ("N habits will fire today — already on your timeline") appears in Phase 1 when any are present. The TodoistPanel renders a non-actionable "🔁 Habit" label in place of the Link button on rows whose Todoist id backs a `TodaysHabitInstance`. **v6.2:** each intention row also shows two icon buttons — `📥` (Move to backlog) and `🗑` (Delete, confirm-modal). Both unschedule the intention's linked Todoist tasks via the shared `useIntentionRemoval` hook.

2. **Step 2 — Refine.** Per-intention sequential flow. For each linked task: categorize as **main** or **background**, and set an **estimate** (preset pills: 15m / 30m / 45m / 1hr, or custom). Background tasks clamp to `taskCapDefaults.manualBackground` (default 30 min). Tasks > 60 min trigger a non-blocking nudge to break down via the TodoistPanel (collapsed by default; auto-opens on >60min estimates). **v6.3:** stabilizer habits are no longer in `linkedTasks` at all, so this step is purely about user-task refinement.

3. **Step 3 — Schedule.** Two phases:
   - **Phase 1 (Assign):** A proportional `SessionTimelineBar` shows sessions as blocks plus a dedicated **habit lane** above the sessions where `TodaysHabitInstance` rows render at their `targetTime`. Untimed habits cluster as "Anytime today" chips above the time axis. User clicks a session and assigns tasks to it. Main tasks are exclusive to one session; background tasks can be assigned to multiple. **v6.3:** habits are NOT session-scoped — the previous "Unassigned habits" tray and "🔁 Habits" session group are gone. **v6.2:** the top of Phase 1 shows a "Today's intentions (N)" overview panel listing every active intention with task count + `📥` (Move to backlog) + `🗑` (Delete) buttons — for the overcommitment-realization moment. Cannot advance until at least one task is assigned.
   - **Phase 2 (Time):** Side-by-side TodoistPanel + Google Calendar embed for user-task time-blocking, **plus a "Today's habits" panel** above the split. Lenient stabilizers whose target window has passed get an inline reschedule affordance (time-picker → dispatches `RESCHEDULE_HABIT_INSTANCE`, generating a successor instance with the new time while leaving the recurring Todoist task untouched).
...
4. **Timeline** — `SessionTimelineBar` (read-only) with a pulse on the active session and (v6.3) a habit lane above the session blocks rendering `TodaysHabitInstance`s at their `targetTime`.
5. **`HabitInstanceCard`** (v6.3) — lists today's stabilizer instances with per-row Start/Stop/Complete/Skip/Reschedule controls. Replaces the old session-scoped 🔁 Habits grouping.
6. **Current Session** — detailed card with the active session's tasks: drag-to-reorder, completion checkboxes (with `canvas-confetti` on completion), nudge banners for background tasks. v6.3: each TaskRow has a ▶/■ Start/Stop button for engagement tracking.
...
**Intentions own LinkedTasks.** An intention has `linkedTaskIds: string[]` (ordered Todoist IDs). v6.3: a `LinkedTask` always has an `intentionId` back-reference (stabilizer habits no longer live here). Plus `type` + `estimatedMinutes` + `assignedSessions[]` + `completed` + `titleSnapshot?` + (v6.3) `status?` + `engagement?` + `rescheduledFromTodoistId?` + `rescheduledAt?`.

**Stabilizers live separately on `DayPlan.todaysHabits`.** v6.3: a `TodaysHabitInstance` is the day-of carrier for a stabilizer firing today — `{ id, habitId, todoistTaskId, titleSnapshot, durationMinutes, targetTime?, status, engagement?, rescheduledFromId? }`. Positioned on the timeline by `targetTime`; untimed instances form an "Anytime today" cluster. Independent of session assignment and excluded from session capacity arithmetic.

**Tasks (not intentions) are scheduled.** `DayPlan.taskSessions: Record<sessionId, todoistId[]>` is the source of truth for what runs in each session. `LinkedTask.assignedSessions` is a derived mirror kept in sync by the reducer. Habits don't participate.

**LifeContext sits above the day.** A separate persistent state slice (`life: LifeContext` on the provider, persisted to `orchestrate-life-context`) holds `seasons[]`, `habits[]`, `activeSeasonId`, and (v6.2) `backlog?: BacklogEntry[]`. **v6.1+:** stabilizer habits carry `todoistTaskId` (the persistent recurring Todoist task), `targetTime`, `targetDurationMinutes`, and `windowBehavior`. v6.3: the `REFRESH_TODAYS_HABITS` action (fired on Step 1 mount, re-fired when the Todoist cache grows) consumes `computeTodaysHabitInstances(...)` from `lib/habitsTodoistSync.ts` to populate `plan.todaysHabits`. **v6.2:** `BacklogEntry` snapshots a parked intention. v6.3 adds `unfinishedTaskRecords?: Record<todoistId, EngagementRecord>` to preserve engagement memos when an engaged task moves to the backlog.

Consequence: **a single intention can have both main and background tasks** (the v4 fix). v6.3: **stabilizer habits are an entirely separate render path** from user-task LinkedTasks; the `linkedTasks` array contains only intention-bound work.
...
A migration chain (v1 → v2 → v3 → v4 → v4.1 → v5 → v6 → v6.1 → v6.2 → **v6.3**) handles old saved sessions on load. v6 → v6.1 dropped habit-derived intentions and re-anchored their LinkedTasks as orphans. v6.1 → v6.2 added the backlog. **v6.2 → v6.3** harvests any `sourceHabitId` LinkedTasks (legacy orphan habit-tasks) into `plan.todaysHabits` and removes them from `linkedTasks`, prunes their ids from `taskSessions`, and stamps `status` on every remaining LinkedTask. Schema marker is now `6.3` on plan, settings, life, and saved-session payloads.
...
**Habit decoupling + Task engagement (v6.3)**
- **Stabilizer habits are decoupled from sessions.** They live as `TodaysHabitInstance` rows on `plan.todaysHabits`, positioned on the timeline by `targetTime` (or clustered as "Anytime today" when untimed). The `SessionTimelineBar` gains a dedicated habit lane above the session blocks. `HabitInstanceCard` on the dashboard surfaces per-row Start/Stop/Complete/Skip/Reschedule controls.
- **Lenient-habit reschedule generates a clone instance, not a new Todoist task.** When a lenient stabilizer's target window has passed, Step 3 Phase 2 and the dashboard expose a Reschedule affordance. `RESCHEDULE_HABIT_INSTANCE` marks the predecessor `unfinished` (if engaged) or `skipped` (if untouched) and appends a successor `TodaysHabitInstance` with the new `targetTime` and `rescheduledFromId` set. The recurring Todoist task is untouched, so the user's Todoist recurrence stays clean.
- **Task engagement tracking.** Each `LinkedTask` now carries a `status` (`pending | engaged | completed | unfinished`) and optional `engagement: { startedAt, endedAt?, totalMinutes? }`. The dashboard's `TaskRow` gets a ▶/■ Start/Stop button next to the completion checkbox; `START_TASK_ENGAGEMENT` and `STOP_TASK_ENGAGEMENT` actions accumulate minutes across cycles.
- **Backlog carries engagement.** `MOVE_INTENTION_TO_BACKLOG` captures any engaged-but-incomplete LinkedTasks' engagement records into `BacklogEntry.unfinishedTaskRecords`. The Backlog tab renders a `✱ Engaged earlier` annotation. On Bring-to-today, restored LinkedTasks for previously-engaged ids get stamped with `rescheduledFromTodoistId` + `rescheduledAt`.
- **No habit capacity drain.** Habits are excluded from `computeSessionCapacity` — session capacity arithmetic only sees user-assigned `LinkedTask`s.
...
Treat these as future work, not current behavior. **First-class habits and Seasons shipped in v5; the Habit.kind split, Light Pool, True Rest, capacity arithmetic, and the legacy `isHabit` purge shipped in v6; the habit-as-orphan-task decoupling shipped in v6.1; the intentions backlog + Todoist unschedule-on-discard shipped in v6.2; the full habit/session decoupling (TodaysHabitInstance), engagement tracking, and clone-on-reschedule primitive shipped in v6.3** (see [history/plan_v5.md](./history/plan_v5.md), [history/plan_v6.md](./history/plan_v6.md), and [history/plan_v6.x.md](./history/plan_v6.x.md)).
