...
- **Reconcile on Dashboard mount.** Not needed today because the daily-rollover path forces Step 1 before Dashboard. Would become necessary only if `setupComplete` were ever skippable or if rollover stopped resetting the plan — neither is on the roadmap. *(Superseded by [v6.5](./plan_v6.5.md): reconcile is now a central provider that fires app-wide, so the Step-1-only limitation no longer applies.)*
- **Even reconcile surface across surfaces.** This iteration left the overdue reconcile auto-firing only from Step 1, while the needs-sync repair stayed a manual button inside `/habits`. Unifying the two was scoped out here and picked up in [v6.5](./plan_v6.5.md).
...
[v6.5 — Central Stabilizer Reconciliation](./plan_v6.5.md) completes the reconcile story this iteration started. v6.4 fixed *what* reconcile does (bump overdue tasks, advance on skip) but left it firing unevenly — overdue auto-ran only on Step 1 mount, needs-sync was a manual `/habits` button. v6.5 extracts both into a single `ReconciliationProvider` with consistent triggers (first hydration + focus + manual) and cross-app visibility (`HabitSyncChip`). Read the two together: v6.4 is the behavioral fix, v6.5 is the architectural consolidation.
