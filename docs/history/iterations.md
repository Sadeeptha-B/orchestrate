...
**Habits reschedule in place.** `RESCHEDULE_HABIT_INSTANCE` updates the instance's `targetTime` directly and stamps `rescheduledAt`. Status, engagement record, and id are preserved — no clone, no strikethrough trail, no Todoist write. One instance per habit per day. Both the dashboard `HabitInstanceCard` and the Step 3 habits panel (rendered in both Phase 1 and Phase 2) expose a ⤴ Reschedule affordance on every active instance, since Step 3 IS the planning step and the user should be able to set/change times regardless of whether the window has elapsed. Habit-form edits to `targetTime` / `durationMinutes` propagate into the existing planned instance via the merge logic in `REFRESH_TODAYS_HABITS` (which preserves the user-chosen time when `rescheduledAt` is set).
