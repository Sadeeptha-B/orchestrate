...
- **[docs/reference/cloudflare_workers.md](../../reference/cloudflare_workers.md)** — the *conceptual "how it works"* reference: the three trust zones, the Functions table, the OAuth flow step-by-step, every variable/secret and who uses it, the KV key shapes, and the full security model.
...
- **Docs:** updated synthesis (§2 Crypto, §7 Todoist row + hosting paragraph, §11, §14), data-model (Todoist token + Google entries, `RESET_ALL` note), deployment.md (Part C Todoist connect), and reference/cloudflare_workers.md (the Todoist proxy walkthrough). Verified via `npm run build` + `npm run lint`.
