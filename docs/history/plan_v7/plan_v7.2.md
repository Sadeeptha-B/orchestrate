...
- **Docs:** updated synthesis (§2 Crypto, §7 Todoist row + hosting paragraph, §11, §14), data-model (Todoist token + Google entries, `RESET_ALL` note), deployment.md (Part C Todoist connect), and reference/cloudflare_workers.md (§9 Todoist proxy). Verified via `npm run build` + `npm run lint`.
