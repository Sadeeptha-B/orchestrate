...
Focus Mode renders the **whole trail** in the **per-task focus timeline** (`TaskFocusTimeline`, v7.7) beside the timer — one node per engagement segment with its `exit` note and timestamp, the `entry` note capping the top; notes are deletable there (`DELETE_TASK_CONTEXT_NOTE`). The latest note (`contextTrail.at(-1)`) is the task's current "start here", surfaced as a truncated `↩` preview in the **Focus picker** (v7.7 — moved off the planning dashboard, since it's execution metadata). A mid-session **"+ Add breadcrumb"** in the Focus `stopping` view appends an `exit` note (`APPEND_TASK_CONTEXT_NOTE`) without ending the segment. Captured into `BacklogEntry.contextTrails` on park, restored on bring-to-today.

**Deliberate gates (v7.4 Phase 2 / configurable since v7.5).** The note gates are governed by `settings.focusStrict` (default true). In **strict** mode re-entry context is mandatory at the session boundaries: you **cannot Start** a context-less task without naming a first concrete action (captured *in Focus* via `UPSERT_TASK_ENTRY_NOTE` — v7.6 inline `firstAction` phase, not a modal), you **cannot Stop / leave Focus / switch tasks** without a next-step note, and the dashboard ▶ lands directly in Focus. In **relaxed** mode both notes are optional and the dashboard ▶/■ start and stop the timer in place. See `synthesis.md` §3.3 for the state machine.
