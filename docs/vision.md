...
- **Habit** — a first-class recurring entity discriminated by `kind`: `stabilizer` (synced to Todoist as a recurring task, surfaced as a `TodaysHabitInstance` on the day's timeline independent of session assignment) or `light-coherent` (Light Pool micro-gap fillers). Owns recurrence rule, minimum-viable form, trigger cue, anchor flag, and season scope.
