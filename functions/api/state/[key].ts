...
// Identity-guarded; rows are scoped to the caller's user_id. Upserts one slice with a
// last-write-wins guard applied *inside* the SQL statement (race-safe without a transaction): the
// write only lands when the incoming updatedAt is >= the stored one. A rejected write returns 409
// with the current row so the client can back off — the next cold-start merge (SyncGate)
// reconciles authoritatively.

import { SYNC_SLICE_KEYS, type StateEnv, json, requireUser } from '../../_shared';
...
export const onRequestPut: PagesFunction<StateEnv> = async ({ request, env, params }) => {
    ...
    try {
        ...
        const res = await env.SYNC_DB.prepare(
            `INSERT INTO slices (user_id, key, value, schema_version, updated_at) VALUES (?1, ?2, ?3, ?4, ?5)
             ON CONFLICT(user_id, key) DO UPDATE SET
                 value = excluded.value,
                 schema_version = excluded.schema_version,
                 updated_at = excluded.updated_at
             WHERE excluded.updated_at >= slices.updated_at`,
        ).bind(auth.email, key, value, schemaVersion, updatedAt).run();

        if (res.meta.changes === 0) {
            const current = await env.SYNC_DB
                .prepare('SELECT value, schema_version, updated_at FROM slices WHERE user_id = ?1 AND key = ?2')
                .bind(auth.email, key)
                .first<CurrentRow>();
            return json(
                {
