...
// Scenes share one width so the expanded card doesn't resize when the activity changes; the
// widget sizes the ambient card off the backdrop (`w-min`), so this is the diorama's width.
const SCENE_W = 39;
const GROUND = '▁'.repeat(SCENE_W);
