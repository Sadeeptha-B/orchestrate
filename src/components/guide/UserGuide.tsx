          ...
          On matching days it offers a "+ Add" chip in Step 1 that seeds an intention.`}</Flow>
                    ...
                    <Section id="typical-day" title="11. A typical day, start to finish">
                        ...
                        <ul className="list-disc pl-5 space-y-1">
                            ...
                            <li><strong>Calendar write-back + blocklists (optional):</strong> when Google Calendar is connected, Orchestrate keeps a dedicated <em>"Orchestrate"</em> calendar (rename it in Settings → Integrations) and the <strong>Sync</strong> button (on the timeline / calendar) writes your sessions to it. If you use the <a href="https://www.nodistraction.net" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">No Distraction</a> extension, add your blocklist suffixes in Settings and pick one per session — it's appended to the session's calendar event (e.g. <code>Afternoon Session -ND</code>). When the session starts, the dashboard asks you to confirm the blocklist; once confirmed it's locked until the session ends.</li>
                        </ul>
